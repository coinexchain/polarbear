...
func Ecdh(pubkey, seckey []byte) ([]byte, error) {
	if len(pubkey) == 0 {
	...
	var result [32]byte
	resultdata := (*C.uchar)(unsafe.Pointer(&result[0]))
	pubkeydata := (*C.uchar)(unsafe.Pointer(&pubkey[0]))
	seckeydata := (*C.uchar)(unsafe.Pointer(&seckey[0]))
	ret := C.secp256k1_ext_ecdh(context, resultdata, pubkeydata, C.ulong(len(pubkey)), seckeydata)
	if ret != 1 {
		return nil, ErrInvalidKey
	}
	return result[:], nil
